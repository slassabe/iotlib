...
class MQTTBridge(IMQTTBridge):
    ...
    def __init__(self, mqtt_service: IMQTTService, codec: ICodec):
        """
        ...
        _client.message_callback_add(
            self.codec.get_availability_topic(), self._avalability_callback
        )
        for _topic_property in self.codec.get_subscription_topics():
            _client.message_callback_add(_topic_property, self._value_callback)
    ...
    def __repr__(self) -> str:
        def get_attrs_and_values(obj: Any) -> list[tuple[str, Any]]:
            """
            Returns a list of tuples containing attribute names and their corresponding values.

            :param obj: The object whose attributes and values are to be retrieved.
            :return: A list of tuples containing attribute names and their corresponding values.
            """
            return [
        ...
        attrs_and_values = get_attrs_and_values(self)
        formatted_attrs = [f"{attr} : {val}" for attr, val in attrs_and_values]
        result = f"{self.__class__.__name__} ({' | '.join(formatted_attrs)})"
        return result
    ...
    def _avalability_callback(
        ...
        try:
            ...
            iotlib_logger.exception(
                '"[%s]" : Exception occurred decoding : %s / %s',
                exp,
                message.topic,
                payload[:100],
            )
    ...
    def _value_callback(
        ...
        try:
            ...
            iotlib_logger.exception(
                '"[%s]" : Exception occured decoding : %s / %s',
                exp,
                message.topic,
                payload[:100],
            )
    ...
    def _on_disconnect_callback(  # pylint: disable=too-many-arguments
        ...
        if reason_code == 0:
            iotlib_logger.debug(
                "Disconnection occures - rc : %s -> stop loop", reason_code
            )
            client.loop_stop()
        else:
            iotlib_logger.warning(
    ...
    def _handle_on_subscribe(  # pylint: disable=too-many-arguments
        ...
        try:
            self._on_subscribe_callback(
                client, userdata, mid, reason_code_list, properties
            )
        except Exception as error:
            iotlib_logger.exception("Failed handling subscribe %s", error)
    ...
    def _handle_values(self, topic: str, payload: bytes) -> None:
        ...
        for _handler in self.codec.get_message_handlers(topic):
            ...
            # Decode value
            _decoded_value = _decoder(
                self.codec, topic, self.codec.fit_payload(payload)
            )
            # Process handle_value with the decoded value
            _result = _virtual_device.handle_value(_decoded_value)
            iotlib_logger.debug("[%s] handle_value result : %s", self, _result)
    ...
    def _handle_availability(self, payload: str) -> bool:
        ...
        iotlib_logger.debug("Handle availability message with payload: %s", payload)
        new_avail = self.codec.decode_avail_pl(payload)
        if self.availability != new_avail:
            self._availability = new_avail
            iotlib_logger.debug("Availability updated: %s", self.availability)
            # Notify
            _device_name = self.codec.device_name
            for _processor in self._availability_processors:
                _processor.process_availability_update(self.availability, _device_name)
        else:
            iotlib_logger.debug("Availability unchanged: %s", self.availability)
        return new_avail
