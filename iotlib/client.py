...
class MQTTClient(IMQTTService):
    ...
    def __post_init__(self) -> None:
        ...
        self._mqtt_client.username_pw_set(self.user_name, self.user_pwd)
    ...
    def __repr__(self) -> str:
        return (
            ...
            f'"{self._mqtt_client._client_id}" | '
    ...
    def stop(self) -> mqtt.MQTTErrorCode:
        ...
        if not self._connected:
            iotlib_logger.error("[%s] Unable to stop disconnected client", self)
    ...
    def connect(
        ...
        try:
            ...
            iotlib_logger.debug("[%s] Connection request returns : %s", self, _rc)
            return _rc
    ...
    def _handle_on_connect(
        ...
        for on_connect_handler in self.on_connect_handlers:
            try:
                ...
                iotlib_logger.exception("Failed handling connect %s", error)
    ...
    def _handle_on_disconnect(
        ...
        for on_disconnect_handler in self.on_disconnect_handlers:
            try:
                ...
                iotlib_logger.exception("Failed handling disconnect %s", error)
